...
namespace RavaSync.API.SignalR;
...
public interface IMareHub
{
    ...
    Task Client_UserReceivePairRequest(PairRequestDto dto);
    Task Client_UserPairRequestResult(PairRequestResultDto dto);
    ...
    Task<VenueInfo?> VenueLookup(string canonicalKey);
    Task<VenueInfo?> VenueRegister(string shellGid, RegisterVenueRequest req);

    Task<GroupJoinInfoDto> VenueJoin(GroupPasswordDto passwordedGroup);
    Task<bool> VenueJoinFinalize(GroupJoinDto passwordedGroup);
    Task UserSendPairRequest(string targetIdent, string requesterInGameName);
    Task UserRespondPairRequest(string requesterUid, bool accepted);

    Task MeshRegister(string sessionId);
    Task MeshSend(MeshMessageDto message);
    Task Client_MeshMessage(MeshMessageDto dto);
}