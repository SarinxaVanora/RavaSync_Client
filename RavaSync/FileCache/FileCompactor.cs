...
public sealed class FileCompactor
{
    ...
    private const long MaxSingleFileBytes = 400L * 1024 * 1024; // 400 MiB

    public long GetFileSizeOnDisk(FileInfo fileInfo, bool? isNTFS = null)
    {
        // Decide if this drive is NTFS (or use override, if provided)
        bool ntfs = isNTFS ?? string.Equals(
            new DriveInfo(fileInfo.FullName).DriveFormat,
            "NTFS",
            StringComparison.OrdinalIgnoreCase);

        // On Wine or non-NTFS, just use the logical file length
        ...
        // Query compressed size from Windows
        uint hi;
        uint lo = GetCompressedFileSizeW(fileInfo.FullName, out hi);

        // Combine high + low into a 64-bit value safely
        var rawSize = (long)((ulong)hi << 32 | lo);

        // - size <= 0  → clearly bogus
        // - size > 400 MiB → impossible for your cache model
        if (rawSize <= 0 || rawSize > MaxSingleFileBytes)
        {
            // Fall back to logical length as a safe, bounded value
            return fileInfo.Length;
        }
