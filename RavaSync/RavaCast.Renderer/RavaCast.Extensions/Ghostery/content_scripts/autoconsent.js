(function() {
	...
	var snippets = {
		...
		EVAL_DIDOMI_TEST: () => {
			...
			if (purposes) return Object.values(purposes).some((p) => !p.enabled);
	...
	var TrustArcFrame = class extends AutoConsentCMPBase {
		...
		async optOut() {
			...
			if (await this.click(".rejectAll,.declineAllButtonLower", true)) return true;
	...
	var SourcePoint = class extends AutoConsentCMPBase {
		...
		async optOut() {
			...
			if (!this.isManagerOpen()) {
				...
				if (this.elementVisible(".sp_choice_type_13", "any")) return await this.click(".sp_choice_type_13");
	...
	var Onetrust = class extends AutoConsentCMPBase {
		...
		async optOut() {
			await this.wait(500);
			if (this.elementVisible("#onetrust-reject-all-handler", "any")) return await this.click("#onetrust-reject-all-handler");
			if (this.elementVisible(".ot-pc-refuse-all-handler", "any")) return await this.click(".ot-pc-refuse-all-handler");
			if (this.elementVisible(".js-reject-cookies", "any")) return await this.click(".js-reject-cookies");
			if (this.elementVisible(".onetrust-close-btn-handler", "any")) {
				...
				if (document.getElementById("onetrust-banner-sdk")?.classList.contains("ot-close-btn-link") && !this.elementExists("#onetrust-accept-btn-handler,#onetrust-reject-all-handler,#onetrust-pc-btn-handler,.ot-sdk-show-settings,button.js-cookie-settings")) return await this.click(".onetrust-close-btn-handler");
			}
		...
		*/
		...
		async waitForPopup(cmp, retries = 10, interval = 500) {
			...
			if (!isOpen && retries > 0) {
				...
				else await this.domActions.wait(interval);
