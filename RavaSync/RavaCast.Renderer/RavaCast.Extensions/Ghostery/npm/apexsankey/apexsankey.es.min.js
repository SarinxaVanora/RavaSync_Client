...
//#region node_modules/apexsankey/apexsankey.es.min.js
var Export = class {
	...
	getSvgString() {
		return this.canvas.svg().replace(/(<img [\w\W]+?)(>)/g, "$1 />").replace(/(<br)(>)/g, "$1 />").replace(/(<hr)(>)/g, "$1 />");
	}
	svgUrl() {
...
var rsApos$1 = "['’]", rsBreak = "[" + rsBreakRange + "]", rsCombo = "[" + rsComboRange + "]", rsDigits = "\\d+", rsDingbat = "[" + rsDingbatRange + "]", rsLower = "[" + rsLowerRange + "]", rsMisc = "[^" + rsAstralRange + rsBreakRange + rsDigits + rsDingbatRange + rsLowerRange + rsUpperRange + "]", rsModifier = "(?:" + rsCombo + "|\\ud83c[\\udffb-\\udfff])", rsNonAstral = "[^" + rsAstralRange + "]", rsRegional = "(?:\\ud83c[\\udde6-\\uddff]){2}", rsSurrPair = "[\\ud800-\\udbff][\\udc00-\\udfff]", rsUpper = "[" + rsUpperRange + "]", rsZWJ = "\\u200d";
var rsMiscLower = "(?:" + rsLower + "|" + rsMisc + ")", rsMiscUpper = "(?:" + rsUpper + "|" + rsMisc + ")", rsOptContrLower = "(?:" + rsApos$1 + "(?:d|ll|m|re|s|t|ve))?", rsOptContrUpper = "(?:" + rsApos$1 + "(?:D|LL|M|RE|S|T|VE))?", reOptMod = rsModifier + "?", rsOptVar = "[" + rsVarRange + "]?", rsOptJoin = "(?:" + rsZWJ + "(?:" + [
	rsNonAstral,
	rsRegional,
	rsSurrPair
...
var updateTooltip = (id = "", styles, content = "") => {
	...
	if (styles) tooltipElement?.setAttribute("style", styles);
	else tooltipElement?.removeAttribute("style");
	if ((tooltipElement == null ? void 0 : tooltipElement.innerHTML.replaceAll("'", "\"")) !== content.replaceAll("'", "\"")) tooltipElement && (tooltipElement.innerHTML = content);
};
var getTooltipStyles = ({ bgColor, borderColor, maxWidth, padding, x, y }) => {
	const styles = [
		"position: absolute;",
		`left: ${x + 20}px;`,
		`top: ${y + 20}px;`,
	...
	if (padding !== void 0) styles.push(`padding: ${padding}px`);
	return styles.join(" ");
};
...
function drawRect({ borderColor = "#caced0", color = "#97cbe6", height = 0, opacity = 1, radius = 0, width = 0, x = 0, y = 0 } = {}) {
	const rect = new Rect();
	rect.attr({
		height,
		opacity,
...
function drawText(text = "", { fontColor = "#666767", pointerEvents = "all", textAnchor = "start", x, y }) {
	...
	textSvg.tspan(text);
	if (x !== void 0 && y !== void 0) textSvg.move(x, y);
	textSvg.attr({
...
function drawPath(pathString, { edgeColor, id = "", opacity = 1 } = {}) {
...
var Toolbar = class {
	...
	render({ enableExport = false, enableFitscreen = false, enableZoom = false, onFitscreen = noop, onZoom = noop }) {
		var _a;
...
var isBuffer$1 = (Buffer ? Buffer.isBuffer : void 0) || stubFalse;
...
function arrayLikeKeys(value, inherited) {
	...
	for (var key in value) if (hasOwnProperty$3.call(value, key) && !(skipIndexes && (key == "length" || isBuff && (key == "offset" || key == "parent") || isType && (key == "buffer" || key == "byteLength" || key == "byteOffset") || isIndex(key, length)))) result.push(key);
	return result;
}
...
function linkTypeOrder(G2, u) {
	...
	const dirs = new Map(Array.from(edgesByType.entries()).map(([k, v]) => {
		...
		return [k, sumBy(v, (e) => G2.edge(e).value * otherY(e)) / total];
	}));
	const mo = Array.from(dirs.keys());
	mo.sort();
	return mo;
	function otherY(e) {
		if (e.v === u) return G2.node(e.w).y;
		if (e.w === u) return G2.node(e.v).y;
		throw new Error();
	}
}
...
function compareDirection(G2, clockwise = true) {
	return function(a, b) {
		...
		if (a.v === b.v && a.w === b.w) {
			...
			else if (typeof a.name === "string" && typeof b.name === "string") return a.name.localeCompare(b.name);
			return 0;
		}
		if (Math.abs(da - db) < .001) {
			if (a.v && a.w && b.v && b.w) {
				if (a.w === b.w) return -1 * (G2.node(a.v).y - G2.node(b.v).y);
				else if (a.v === b.v) return -1 * (G2.node(a.w).y - G2.node(b.w).y);
				return 0;
			}
			return 0;
		}
...
function compareDirectionGroupingTypes(G2, mo, clockwise = true) {
	return function(a, b) {
		if (a.name !== b.name) return mo.indexOf(a.name) - mo.indexOf(b.name);
		const da = linkDirection(G2, a);
		const db = linkDirection(G2, b);
		const c = clockwise ? 1 : -1;
		if (Math.abs(da - db) < .001) {
			if (a.v && a.w && b.v && b.w) {
				if (a.w === b.w) return c * (da > 0 ? -1 : 1) * (G2.node(a.v).y - G2.node(b.v).y);
				else if (a.v === b.v) return c * (da > 0 ? -1 : 1) * (G2.node(a.w).y - G2.node(b.w).y);
				return 0;
			}
			return 0;
		}
...
function findFirst(links, p) {
	let jmid = null;
	for (let j = 0; j < links.length; ++j) if (p(links[j])) {
		jmid = j;
		break;
	}
	return jmid;
...
function sweepCurvatureInwards(links, rr) {
	...
	for (let i = links.length - 1; i >= 0; --i) {
		h = links[i].dy / 2;
		if (links[i][rr] - h < Rmin) links[i][rr] = Math.min(links[i].Rmax, Rmin + h);
		Rmin = links[i][rr] + h;
	}
	let Rmax = links[0].Rmax + links[0].dy / 2;
	for (let i = 0; i < links.length; ++i) {
		h = links[i].dy / 2;
		if (links[i][rr] + h > Rmax) links[i][rr] = Math.max(h, Rmax - h);
		Rmax = links[i][rr] - h;
	}
}
...
function maximumRadiusOfCurvature(link) {
	...
	if (link.d0 !== link.d1) return Math.abs(Dy) / 2.1;
...
function setEdgeEndCurvatures(edges, rr) {
	edges.forEach((edge) => {
		...
		edge[rr] = Math.max(edge.dy / 2, edge.d0 === edge.d1 ? edge.Rmax * .6 : 5 + edge.dy / 2);
	});
...
function addDummyNodes(G2) {
	G2.edges().forEach((e) => {
		...
		const dummyRanks = [];
		if (r + 1 <= W.rank) {
			if (V.direction === "l") dummyRanks.push(r);
			while (++r < W.rank) dummyRanks.push(r);
			if (W.direction === "l") dummyRanks.push(r);
			replaceEdge(G2, e, edge, dummyRanks, "r");
		} else if (r > W.rank) {
			if (V.direction !== "l") dummyRanks.push(r);
			while (r-- > W.rank + 1) dummyRanks.push(r);
			if (W.direction !== "l") dummyRanks.push(r);
...
function replaceEdge(G2, oldEdge, oldLabel, dummyRanks, direction) {
	...
	nn.forEach((n, i) => {
		if (i + 1 < nn.length) G2.setEdge(nn[i], nn[i + 1], {
...
function removeDummyNodes(G2) {
	G2.nodes().filter((node) => node.startsWith("__")).forEach((dNode) => {
		var _a;
		...
		while (node.dummy) {
			...
			dummyEdges.forEach((dummyEdge, i) => {
				if (!dummyEdge.origLabel.points) dummyEdge.origLabel["points"] = [];
...
function countBetweenCrossings(G2, orderA, orderB) {
	...
	north.forEach((u) => {
		south.forEach((v, j) => {
			if (G2.hasEdge(u, v) || G2.hasEdge(v, u)) southSeq.push(j);
		});
	});
	let firstIndex = 1;
...
function require_assocIndexOf() {
	...
	function assocIndexOf2(array, key) {
		var length = array.length;
		while (length--) if (eq2(array[length][0], key)) return length;
		return -1;
	}
...
function require_listCacheDelete() {
	...
	function listCacheDelete2(key) {
		...
		if (index < 0) return false;
		if (index == data.length - 1) data.pop();
		else splice2.call(data, index, 1);
		--this.size;
		return true;
	}
...
function require_root() {
	...
	_root = freeGlobal2 || freeSelf2 || Function("return this")();
	return _root;
}
var _Symbol;
...
function require_getRawTag() {
	...
	function getRawTag2(value) {
		...
		if (unmasked) if (isOwn) value[symToStringTag2] = tag;
		else delete value[symToStringTag2];
		return result;
	}
...
function require_baseGetTag() {
	...
	function baseGetTag2(value) {
		if (value == null) return value === void 0 ? undefinedTag2 : nullTag2;
...
function require_baseIsNative() {
	...
	function baseIsNative2(value) {
		if (!isObject2(value) || isMasked2(value)) return false;
		return (isFunction2(value) ? reIsNative2 : reIsHostCtor2).test(toSource2(value));
	}
...
function require_assignValue() {
	...
	function assignValue(object, key, value) {
		...
		if (!(hasOwnProperty2.call(object, key) && eq2(objValue, value)) || value === void 0 && !(key in object)) baseAssignValue(object, key, value);
	}
...
function requireIsArguments() {
	...
	isArguments_1 = baseIsArguments2(/* @__PURE__ */ function() {
		return arguments;
...
function requireIsBuffer() {
	...
	(function(module2, exports2) {
		...
		var freeModule2 = freeExports2 && module2 && !module2.nodeType && module2;
		var Buffer2 = freeModule2 && freeModule2.exports === freeExports2 ? root2.Buffer : void 0;
		module2.exports = (Buffer2 ? Buffer2.isBuffer : void 0) || stubFalse2;
...
function require_nodeUtil() {
	...
	(function(module2, exports2) {
		...
		var freeExports2 = exports2 && !exports2.nodeType && exports2;
		var freeModule2 = freeExports2 && module2 && !module2.nodeType && module2;
		var freeProcess2 = freeModule2 && freeModule2.exports === freeExports2 && freeGlobal2.process;
...
function requireIsTypedArray() {
	...
	isTypedArray_1 = nodeIsTypedArray2 ? baseUnary2(nodeIsTypedArray2) : baseIsTypedArray2;
...
function require_arrayLikeKeys() {
	...
	function arrayLikeKeys2(value, inherited) {
		...
		for (var key in value) if ((inherited || hasOwnProperty2.call(value, key)) && !(skipIndexes && (key == "length" || isBuff && (key == "offset" || key == "parent") || isType && (key == "buffer" || key == "byteLength" || key == "byteOffset") || isIndex2(key, length)))) result.push(key);
		return result;
	}
...
function require_isPrototype() {
	...
	function isPrototype2(value) {
		var Ctor = value && value.constructor;
		return value === (typeof Ctor == "function" && Ctor.prototype || objectProto2);
	}
...
function require_nativeKeys() {
	...
	_nativeKeys = require_overArg()(Object.keys, Object);
	return _nativeKeys;
}
var _baseKeys;
...
function require_baseKeys() {
	...
	function baseKeys2(object) {
		if (!isPrototype2(object)) return nativeKeys2(object);
		var result = [];
		for (var key in Object(object)) if (hasOwnProperty2.call(object, key) && key != "constructor") result.push(key);
		return result;
	}
	_baseKeys = baseKeys2;
	return _baseKeys;
...
function require_baseKeysIn() {
	...
	function baseKeysIn(object) {
		...
		for (var key in object) if (!(key == "constructor" && (isProto || !hasOwnProperty2.call(object, key)))) result.push(key);
		return result;
	}
...
function require_cloneBuffer() {
	...
	(function(module2, exports2) {
		...
		var freeExports2 = exports2 && !exports2.nodeType && exports2;
		var freeModule2 = freeExports2 && module2 && !module2.nodeType && module2;
		var Buffer2 = freeModule2 && freeModule2.exports === freeExports2 ? root2.Buffer : void 0, allocUnsafe = Buffer2 ? Buffer2.allocUnsafe : void 0;
...
function require_getSymbols() {
	...
	_getSymbols = !nativeGetSymbols2 ? stubArray2 : function(object) {
		if (object == null) return [];
		object = Object(object);
...
function require_getPrototype() {
	...
	_getPrototype = require_overArg()(Object.getPrototypeOf, Object);
...
function require_getSymbolsIn() {
	...
	_getSymbolsIn = !Object.getOwnPropertySymbols ? stubArray2 : function(object) {
		var result = [];
		while (object) {
...
function require_DataView() {
	...
	_DataView = require_getNative()(require_root(), "DataView");
	return _DataView;
}
var _Promise;
...
function require_Promise() {
	...
	_Promise = require_getNative()(require_root(), "Promise");
	return _Promise;
}
var _Set;
var hasRequired_Set;
function require_Set() {
	...
	hasRequired_Set = 1;
	_Set = require_getNative()(require_root(), "Set");
	return _Set;
}
var _WeakMap;
...
function require_WeakMap() {
	...
	_WeakMap = require_getNative()(require_root(), "WeakMap");
	return _WeakMap;
}
var _getTag;
...
function require_getTag() {
	...
	if (DataView2 && getTag2(new DataView2(/* @__PURE__ */ new ArrayBuffer(1))) != dataViewTag2 || Map2 && getTag2(new Map2()) != mapTag2 || Promise2 && getTag2(Promise2.resolve()) != promiseTag2 || Set2 && getTag2(new Set2()) != setTag2 || WeakMap2 && getTag2(new WeakMap2()) != weakMapTag2) getTag2 = function(value) {
		var result = baseGetTag2(value), Ctor = result == objectTag2 ? value.constructor : void 0, ctorString = Ctor ? toSource2(Ctor) : "";
		if (ctorString) switch (ctorString) {
			case dataViewCtorString2: return dataViewTag2;
			case mapCtorString2: return mapTag2;
			case promiseCtorString2: return promiseTag2;
			case setCtorString2: return setTag2;
			case weakMapCtorString2: return weakMapTag2;
		}
		return result;
	};
	_getTag = getTag2;
...
function require_Uint8Array() {
	...
	_Uint8Array = require_root().Uint8Array;
	return _Uint8Array;
}
...
function require_initCloneByTag() {
	...
	function initCloneByTag(object, tag, isDeep) {
		...
		switch (tag) {
			case arrayBufferTag2: return cloneArrayBuffer(object);
			case boolTag2:
			case dateTag2: return new Ctor(+object);
			case dataViewTag2: return cloneDataView(object, isDeep);
			case float32Tag2:
			case float64Tag2:
			case int8Tag2:
			case int16Tag2:
			...
			case uint16Tag2:
			case uint32Tag2: return cloneTypedArray(object, isDeep);
			case mapTag2: return new Ctor();
			case numberTag2:
			case stringTag2: return new Ctor(object);
			case regexpTag2: return cloneRegExp(object);
			case setTag2: return new Ctor();
			case symbolTag2: return cloneSymbol(object);
		}
	}
...
function requireIsMap() {
	...
	isMap_1 = nodeIsMap ? baseUnary2(nodeIsMap) : baseIsMap;
	return isMap_1;
}
var _baseIsSet;
...
function requireIsSet() {
	...
	isSet_1 = nodeIsSet ? baseUnary2(nodeIsSet) : baseIsSet;
	return isSet_1;
}
var _baseClone;
...
function require_baseClone() {
	...
	function baseClone(value, bitmask, customizer, key, object, stack) {
		...
		if (isArr) {
			...
			if (isBuffer2(value)) return cloneBuffer(value, isDeep);
		...
		var props = isArr ? void 0 : (isFull ? isFlat ? getAllKeysIn : getAllKeys2 : isFlat ? keysIn : keys2)(value);
...
function require_baseFor() {
	...
	_baseFor = require_createBaseFor()();
	return _baseFor;
}
var _baseForOwn;
...
function require_baseEach() {
	...
	_baseEach = require_createBaseEach()(baseForOwn);
	return _baseEach;
}
var identity_1;
...
function requireForEach() {
	...
	function forEach(collection, iteratee) {
		return (isArray2(collection) ? arrayEach : baseEach)(collection, castFunction(iteratee));
	}
	forEach_1 = forEach;
	return forEach_1;
}
		...
		while (++index < arrLength) {
			...
			if (seen) {
				if (!arraySome2(other, function(othValue2, othIndex) {
					if (!cacheHas2(seen, othIndex) && (arrValue === othValue2 || equalFunc(arrValue, othValue2, bitmask, customizer, stack))) return seen.push(othIndex);
				})) {
					result = false;
					break;
				}
...
function require_equalByTag() {
	...
	function equalByTag2(object, other, tag, bitmask, customizer, equalFunc, stack) {
		switch (tag) {
			...
			case boolTag2:
			case dateTag2:
			case numberTag2: return eq2(+object, +other);
			case errorTag2: return object.name == other.name && object.message == other.message;
			case regexpTag2:
			case stringTag2: return object == other + "";
			case mapTag2: var convert = mapToArray2;
			case setTag2:
				...
				stack["delete"](object);
				return result;
			case symbolTag2: if (symbolValueOf2) return symbolValueOf2.call(object) == symbolValueOf2.call(other);
		}
		return false;
	}
...
function require_equalObjects() {
	...
	function equalObjects2(object, other, bitmask, customizer, equalFunc, stack) {
		var isPartial = bitmask & COMPARE_PARTIAL_FLAG2, objProps = getAllKeys2(object), objLength = objProps.length;
		if (objLength != getAllKeys2(other).length && !isPartial) return false;
		var index = objLength;
		while (index--) {
			var key = objProps[index];
			if (!(isPartial ? key in other : hasOwnProperty2.call(other, key))) return false;
		}
		var objStacked = stack.get(object);
		var othStacked = stack.get(other);
		if (objStacked && othStacked) return objStacked == other && othStacked == object;
...
function require_baseIsEqualDeep() {
	...
	function baseIsEqualDeep2(object, other, bitmask, customizer, equalFunc, stack) {
		...
		if (isSameTag && isBuffer2(object)) {
			if (!isBuffer2(other)) return false;
			objIsArr = true;
			objIsObj = false;
		}
		if (isSameTag && !objIsObj) {
	...
	function baseIsMatch2(object, source, matchData, customizer) {
		...
		while (++index < length) {
			...
			if (noCustomizer && data[2]) {
				...
				if (!(result === void 0 ? baseIsEqual2(srcValue, objValue, COMPARE_PARTIAL_FLAG2 | COMPARE_UNORDERED_FLAG2, customizer, stack) : result)) return false;
			}
		}
		return true;
	}
...
function require_baseMatches() {
	...
	function baseMatches2(source) {
		...
		if (matchData.length == 1 && matchData[0][2]) return matchesStrictComparable2(matchData[0][0], matchData[0][1]);
		return function(object) {
...
function require_isKey() {
	...
	function isKey2(value, object) {
		...
		var type = typeof value;
		if (type == "number" || type == "symbol" || type == "boolean" || value == null || isSymbol2(value)) return true;
...
function require_memoizeCapped() {
	...
	function memoizeCapped2(func) {
		var result = memoize2(func, function(key) {
			if (cache.size === MAX_MEMOIZE_SIZE2) cache.clear();
			return key;
		});
		var cache = result.cache;
		return result;
...
function require_stringToPath() {
	...
	var reEscapeChar2 = /\\(\\)?/g;
	_stringToPath = memoizeCapped2(function(string) {
		var result = [];
		if (string.charCodeAt(0) === 46) result.push("");
...
function require_baseGet() {
	...
	function baseGet2(object, path) {
		...
		var index = 0, length = path.length;
		while (object != null && index < length) object = object[toKey2(path[index++])];
		return index && index == length ? object : void 0;
	}
	_baseGet = baseGet2;
	return _baseGet;
...
function require_baseMatchesProperty() {
	...
	function baseMatchesProperty2(path, srcValue) {
		if (isKey2(path) && isStrictComparable2(srcValue)) return matchesStrictComparable2(toKey2(path), srcValue);
		return function(object) {
...
function requireFilter() {
	...
	function filter(collection, predicate) {
		return (isArray2(collection) ? arrayFilter2 : baseFilter)(collection, baseIteratee2(predicate, 3));
	}
	filter_1 = filter;
	return filter_1;
}
...
function requireIsEmpty() {
	...
	function isEmpty(value) {
		...
		if (tag == mapTag2 || tag == setTag2) return !value.size;
...
function requireMap() {
	...
	function map(collection, iteratee) {
		return (isArray2(collection) ? arrayMap2 : baseMap)(collection, baseIteratee2(iteratee, 3));
	}
	map_1 = map;
	return map_1;
}
...
function require_asciiSize() {
	...
	_asciiSize = require_baseProperty()("length");
	return _asciiSize;
}
var _hasUnicode;
...
function require_unicodeSize() {
	...
	var rsAstralRange2 = "\\ud800-\\udfff", rsComboRange2 = "\\u0300-\\u036f\\ufe20-\\ufe2f\\u20d0-\\u20ff", rsVarRange2 = "\\ufe0e\\ufe0f";
	...
	var reOptMod2 = rsModifier2 + "?", rsOptVar2 = "[" + rsVarRange2 + "]?", rsOptJoin2 = "(?:" + rsZWJ2 + "(?:" + [
		rsNonAstral2,
		rsRegional2,
		rsSurrPair2
	].join("|") + ")" + rsOptVar2 + reOptMod2 + ")*", rsSeq2 = rsOptVar2 + reOptMod2 + rsOptJoin2, rsSymbol = "(?:" + [
		rsNonAstral2 + rsCombo2 + "?",
		rsCombo2,
		rsRegional2,
		rsSurrPair2,
		rsAstral
...
function requireSize() {
	...
	function size(collection) {
		...
		if (isArrayLike2(collection)) return isString(collection) ? stringSize(collection) : collection.length;
		var tag = getTag2(collection);
		if (tag == mapTag2 || tag == setTag2) return collection.size;
...
function requireTransform() {
	...
	function transform(object, iteratee, accumulator) {
		...
		if (accumulator == null) {
			...
			else if (isObject2(object)) accumulator = isFunction2(Ctor) ? baseCreate(getPrototype(object)) : {};
			else accumulator = {};
		}
...
function require_baseSetToString() {
	...
	_baseSetToString = !defineProperty ? identity2 : function(func, string) {
...
function require_setToString() {
	...
	_setToString = require_shortOut()(baseSetToString);
	return _setToString;
}
var _baseRest;
...
function require_createSet() {
	...
	_createSet = !(Set2 && 1 / setToArray2(new Set2([, -0]))[1] == Infinity) ? noop2 : function(values) {
...
function requireUnion() {
	...
	union_1 = baseRest(function(arrays) {
...
var lodash;
if (typeof commonjsRequire === "function") try {
	lodash = {
		clone: requireClone(),
...
Graph$2.prototype.setDefaultNodeLabel = function(newDefault) {
	if (!_$b.isFunction(newDefault)) newDefault = _$b.constant(newDefault);
...
Graph$2.prototype.setNodes = function(vs, value) {
	...
	_$b.each(vs, function(v) {
		if (args.length > 1) self2.setNode(v, value);
		else self2.setNode(v);
	});
	return this;
};
...
Graph$2.prototype.setParent = function(v, parent) {
	...
	if (_$b.isUndefined(parent)) parent = GRAPH_NODE;
	else {
		parent += "";
		for (var ancestor = parent; !_$b.isUndefined(ancestor); ancestor = this.parent(ancestor)) if (ancestor === v) throw new Error("Setting " + parent + " as parent of " + v + " would create a cycle");
		this.setNode(parent);
	}
	this.setNode(v);
	this._removeFromParentsChildList(v);
...
Graph$2.prototype.filterNodes = function(filter) {
	...
	_$b.each(this._nodes, function(value, v) {
		if (filter(v)) copy2.setNode(v, value);
	});
	_$b.each(this._edgeObjs, function(e) {
		if (copy2.hasNode(e.v) && copy2.hasNode(e.w)) copy2.setEdge(e, self2.edge(e));
	});
	var parents = {};
	...
	if (this._isCompound) _$b.each(copy2.nodes(), function(v) {
...
Graph$2.prototype.setPath = function(vs, value) {
	...
	_$b.reduce(vs, function(v, w) {
		if (args.length > 1) self2.setEdge(v, w, value);
		else self2.setEdge(v, w);
		return w;
	});
	return this;
};
Graph$2.prototype.setEdge = function() {
	...
	v = "" + v;
	w = "" + w;
	if (!_$b.isUndefined(name)) name = "" + name;
	var e = edgeArgsToId(this._isDirected, v, w, name);
	if (_$b.has(this._edgeLabels, e)) {
		if (valueSpecified) this._edgeLabels[e] = value;
		return this;
	}
	if (!_$b.isUndefined(name) && !this._isMultigraph) throw new Error("Cannot set a named edge when isMultigraph = false");
	this.setNode(v);
	this.setNode(w);
...
function write(g) {
	...
	if (!_$a.isUndefined(g.graph())) json2.value = _$a.clone(g.graph());
	return json2;
}
function writeNodes(g) {
	return _$a.map(g.nodes(), function(v) {
		...
		var node = { v };
		if (!_$a.isUndefined(nodeValue)) node.value = nodeValue;
		if (!_$a.isUndefined(parent)) node.parent = parent;
		return node;
	});
}
function writeEdges(g) {
	return _$a.map(g.edges(), function(e) {
		...
		if (!_$a.isUndefined(e.name)) edge.name = e.name;
		if (!_$a.isUndefined(edgeValue)) edge.value = edgeValue;
		return edge;
	});
}
function read(json2) {
...
function tarjan$1(g) {
	...
	function dfs2(v) {
		...
		g.successors(v).forEach(function(w2) {
			...
			} else if (visited[w2].onStack) entry.lowlink = Math.min(entry.lowlink, visited[w2].index);
		});
...
function topsort$1(g) {
	...
	var results = [];
	function visit(node) {
		if (_$2.has(stack, node)) throw new CycleException();
	...
	if (_$2.size(visited) !== g.nodeCount()) throw new CycleException();
	return results;
}
...
function initialOrdering(G2) {
	...
	if (start.length === 0) start = G2.nodes().slice(0, 1);
...
function swapNodes(G2, order) {
	...
	while (improved) {
		improved = false;
		for (let i = 0; i < order.length; ++i) for (let j = 0; j < order[i].length - 1; ++j) {
			const count0 = allCrossings$1(G2, order, i);
			transpose(order[i], j, j + 1);
			if (allCrossings$1(G2, order, i) < count0) improved = true;
			else transpose(order[i], j, j + 1);
		}
	}
}
function allCrossings$1(G2, order, i) {
	let count = 0;
	if (i > 0) count += countCrossings(G2, order[i - 1], order[i]);
	if (i + 1 < order.length) count += countCrossings(G2, order[i], order[i + 1]);
	return count;
}
...
function medianValue(positions) {
	...
	else if (positions.length % 2 === 1) return positions[m];
	else if (positions.length === 2) return (positions[0] + positions[1]) / 2;
...
function sortNodes(G2, order, sweepDirection = 1, includeLoops = false) {
	if (sweepDirection > 0) for (let r = 1; r < order.length; ++r) {
	...
	else for (let r = order.length - 2; r >= 0; --r) {
		const medians = /* @__PURE__ */ new Map();
		order[r].forEach((u) => {
			const neighbour = medianValue(neighbourPositions(G2, order, r, r + 1, u, includeLoops));
			medians.set(u, neighbour);
...
function neighbourPositions(G2, order, i, j, u, includeLoops = false) {
	...
	otherRank.forEach((n, i2) => {
		if (G2.nodeEdges(n, u).length > 0) positions.push(i2);
	});
	if (positions.length === 0 && includeLoops) thisRank.forEach((n, i2) => {
		if (G2.nodeEdges(n, u).length > 0) positions.push(i2 + .5);
	});
...
function sortByPositions(arr, order) {
	...
	for (let i = 1; i < arr.length; ++i) for (let k = i; k > 0; --k) {
		let j = k - 1;
...
function ordering(G2, maxIterations = 25) {
	...
	while (i++ < maxIterations) {
		...
		swapNodes(G2, order);
		if (allCrossings(G2, order) < allCrossings(G2, best)) best = copy(order);
	}
...
function allCrossings(G2, order) {
	let count = 0;
	for (let i = 0; i < order.length - 1; ++i) count += countCrossings(G2, order[i], order[i + 1]);
	return count;
}
function copy(order) {
	const result = [];
	for (const rank of order) {
		const r = [];
		result.push(r);
		for (const node of rank) r.push(node);
	}
	return result;
}
...
function positionHorizontally(G2, order, width, minWidths) {
	...
	let dx;
	if (totalMinWidth > width) dx = minWidths.map((w) => width * w / totalMinWidth);
	else {
...
function justifiedPositioning() {
	...
	function position(G2, order) {
		if (scale === null) position.scaleToFit(G2, order);
		...
		order.forEach((rank) => {
			...
			rank.forEach((band, j) => {
				...
				band.forEach((u, i) => {
					...
					if (node.data && node.data.forceY !== void 0) node.y = margin + node.data.forceY * (height - node.dy);
				});
				if (band.length > 0) G2.node(band[band.length - 1]).spaceBelow = Number.MAX_VALUE;
				y += bandHeight;
			});
		});
...
function setNodeValues(G2, edgeValue, nodeDirection) {
	G2.nodes().forEach((n) => {
		...
		let node = G2.node(n);
		if (!node) G2.setNode(n, node = {});
		node.value = Math.max(incoming, outgoing);
		if (node.direction === void 0) node.direction = nodeDirection(node) || "r";
	});
}
...
function groupedGraph(G2, rankSets) {
	...
	let Smin = false;
	for (const set of rankSets) if (set.type === "min") {
		Smin = true;
		break;
	}
	if (!Smin) {
		const sources = G2.sources();
		if (sources.length > 0) rankSets = [{
			nodes: [sources[0]],
			type: "min"
		}, ...rankSets];
		else rankSets = [{
...
var defined$1 = /* @__PURE__ */ getDefaultExportFromCjs(function defined2() {
	for (var i = 0; i < arguments.length; i++) if (typeof arguments[i] !== "undefined") return arguments[i];
});
function assignInitialRanks(G2) {
	...
	while (queue.length > 0) {
		...
		for (const e of G2.outEdges(v)) marked.add(e);
		for (const n of G2.nodes()) if (queue.indexOf(n) < 0 && !seen.has(n) && !G2.inEdges(n).some((e) => !marked.has(e))) queue.push(n);
	}
}
function makeAcyclic(G2, v0) {
	const tree = findSpanningTree(G2, v0);
	G2.edges().forEach((e) => {
		if (nodeRelationship(tree, e.v, e.w) < 0) {
...
function findSpanningTree(G2, v0) {
	...
	const thread = [];
	if (!G2.hasNode(v0)) throw Error("node not in graph");
	doDfs(G2, v0, visited, tree, thread);
	G2.nodes().forEach((u) => {
		if (!visited.has(u)) doDfs(G2, u, visited, tree, thread);
	});
	thread.forEach((u, i) => {
...
function doDfs(G2, v, visited, tree, thread, depth = 0) {
	if (!visited.has(v)) {
		...
		G2.outEdges(v).map((e) => e.w).forEach((w) => {
			if (!visited.has(w)) tree.setEdge(v, w, { delta: 1 });
			doDfs(G2, w, visited, tree, thread, depth + 1);
		});
	}
}
function assignRanks(graph2, rankSets) {
	if (graph2.nodes().length === 0) return;
	...
	graph2.nodes().forEach((u) => {
		if (!graph2.node(u)) graph2.setNode(u, {});
	});
	GG.nodes().forEach((u) => {
...
function moveSourcesRight(GG) {
	GG.edges().forEach((e) => {
		if (GG.edge(e).temp) moveRight(e.w);
	});
	function moveRight(v) {
		const V = GG.node(v);
		const rank = min(GG.outEdges(v).map((e) => GG.node(e.w).rank - GG.edge(e).delta));
		if (rank !== void 0) V.rank = rank;
	}
}
function createGraph(nodes = [], edges = []) {
	const graph2 = new graphlib.Graph({
		directed: true,
		multigraph: true
	});
	edges.forEach((edge) => {
		graph2.setEdge(edge.source, edge.target, {
			data: edge,
			points: []
		}, edge.type);
	});
	...
	nodes.forEach((node) => {
		if (graph2.hasNode(node.id)) {
			...
			if (node.direction !== void 0) graph2.node(node.id).direction = node.direction;
		}
	});
...
function sankey(options) {
	...
	function layout(linksIn = [], nodesIn = [], data = {}) {
		...
		if (!data.order) {
			...
			assignRanks(graph2, data.rankSets || []);
			order = data.order;
			if (order.length > 0 && order[0].length > 0 && !isArray(order[0][0])) order = order.map((rank) => [rank]);
...
var minWidth = (d) => d.dy === 0 ? 0 : 2;
function radiusBounds(d) {
	const Dx = d.x1 - d.x0;
	const Dy = d.y1 - d.y0;
	return [d.dy / 2, (Dx * Dx + Dy * Dy) / Math.abs(4 * Dy)];
}
function segmentPath(edge) {
	...
	if (isNaN(theta) || Math.abs(theta) < .001) {
		theta = 0;
		r0 = 0;
		r1 = 0;
		x2 = x0;
	...
	return `M${[x0, y0 - h]} ${arc(1, r0)}${[x2 + hs, y2 - hc]} L${[x3 + hs, y3 - hc]} ${arc(-1, r1)}${[x1, y1 - h]} L${[x1, y1 + h]} ${arc(1, r1)}${[x3 - hs, y3 + hc]} L${[x2 - hs, y2 + hc]} ${arc(-1, r0)}${[x0, y0 + h]} Z`;
}
function toOrFromElsewherePath(d) {
	const p = d.points[0];
	const h = Math.max(minWidth(d), d.dy) / 2;
	if (p.style === "down-right") return `M${[p.x - 20, p.y - h]} L${[p.x, p.y - h]} L${[p.x, p.y + h]} L${[p.x - 20, p.y + h]} Z`;
	if (p.style === "right-down") return `M${[p.x, p.y - h]} L${[p.x + 20, p.y - h]} L${[p.x + 20, p.y + h]} L${[p.x, p.y + h]} Z`;
	return null;
}
function generatePath(d) {
	var _a;
	if (((_a = d.points) == null ? void 0 : _a.length) === 1) return toOrFromElsewherePath(d);
	let path = "";
	let seg;
...
var nodeTitle = (nodeData, maxLength) => {
	const text = nodeData.title !== void 0 ? nodeData.title : nodeData.id;
	if (maxLength && text.length > maxLength) return `${text.substr(0, 15)}...`;
	return text;
};
...
function textTransform(node, position, nodeWidth) {
	...
	let x;
	if (position.top) x = position.right ? 4 : -4;
	else x = position.right ? 10 + nodeWidth / 2 : -10 + -nodeWidth / 2;
	return {
		x: node.x + x,
		y: node.y + y
	};
...
function renderNodeText(graphNode, { fontColor, maxRank, nodeWidth }) {
	...
	return drawText(nodeTitle(data, position.top ? 25 : 15), {
		fontColor,
		pointerEvents: "none",
...
function renderNode(graphNode, maxRank, options) {
	...
	nodeBody.on("click", () => {
		if (typeof onNodeClick === "function") onNodeClick(graphNode);
	});
	group.add(nodeBody);
	if (graphNode.data && dy > Number.parseInt(fontSize, 10)) {
		const text = renderNodeText(graphNode, {
			fontColor,
			maxRank,
			nodeWidth
		});
...
var Paper = class {
	constructor(element, options) {
		...
		this.canvas = SVG().addTo(element).size("100%", "100%").viewbox(`-${this.spacing} -${this.spacing + this.getYShift()} ${this.width + this.spacing * 2} ${this.height + this.spacing + this.getYShift()}`).attr({ style: this.options.canvasStyle });
	}
	getYShift() {
	...
	clear() {
		this.canvas.clear().viewbox(`-${this.spacing} -${this.spacing + this.getYShift()} ${this.width + this.spacing * 2} ${this.height + this.spacing + this.getYShift()}`);
	}
	resetViewBox() {
		this.canvas.viewbox(`-${this.spacing} -${this.spacing + this.getYShift()} ${this.width + this.spacing * 2} ${this.height + this.spacing + this.getYShift()}`);
	}
...
var SankeyGraph = class extends Paper {
	construct(data) {
	...
	render({ keepOldPosition = false } = {}) {
		var _a, _b;
		const { enableTooltip, fontColor, fontFamily, fontSize, fontWeight, nodeBorderColor, nodeBorderWidth, nodeWidth, onNodeClick, tooltipId } = this.options;
		...
		this.add(mainGroup);
		if (keepOldPosition) this.updateViewBox(oldViewbox.x, oldViewbox.y, oldViewbox.width, oldViewbox.height);
		if (enableTooltip) {
			const tooltipElement = getTooltip(tooltipId);
			(document.body || document.getElementsByTagName("body")[0]).append(tooltipElement);
		}
	}
	renderEdge(edgeObj, group) {
		const { edgeGradientFill, edgeOpacity, enableTooltip, fontColor, fontFamily, fontSize, fontWeight, nodeWidth, tooltipBGColor, tooltipBorderColor, tooltipId, tooltipTemplate } = this.options;
		...
		if (csNode.dummy) edgeLabel.x0 -= nodeWidth / 2;
		if (ctNode.dummy) edgeLabel.x1 += nodeWidth / 2;
		const path = drawPath(generatePath(edgeLabel), {
			edgeColor: this.canvas.gradient("linear", (add) => {
				var _a, _b, _c;
		...
		if (enableTooltip && sourceNode.data && targetNode.data) {
			...
			path.on("mousemove", (e) => {
				...
				(_a = SVG(`[id="${e.target.id}"]`)) == null || _a.front().opacity(1);
				updateTooltip(tooltipId, styles + fontStyle, tooltipContent);
			});
			path.on("mouseout", (e) => {
				var _a;
				updateTooltip(tooltipId);
				(_a = SVG(`[id="${e.target.id}"]`)) == null || _a.front().opacity(edgeOpacity);
			});
		}
		group.add(path);
	}
...
var ApexSankey = class {
	constructor(element, options) {
		...
		} else calculatedWidth = Number(width);
	...
	render(data) {
		...
		this.graph.render();
		if (this.options.enableToolbar) new Toolbar(this.element, this.graph.canvas, "apex-sankey").render({ enableExport: true });
		return this.graph;
	}
};
//#endregion
