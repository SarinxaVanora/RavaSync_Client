...
import { categories, compactNumberFormatter, numberFormatter, shortDateFormatter } from "../../ui/labels.js";
import { HOME_PAGE_URL, WTM_PAGE_URL } from "../../utils/urls.js";
import Resources from "../../store/resources.js";
import { MergedStats } from "../../store/daily-stats.js";
import assets_default from "../settings/assets/index.js";
//#region src/pages/whotracksme/whotracksme.js
/**
...
*/
var MONTH_IN_MS = 720 * 60 * 60 * 1e3;
var DAY_IN_MS = 1440 * 60 * 1e3;
var now = Date.now();
function formatNumber(value) {
	return value >= 1e4 ? compactNumberFormatter.format(value) : numberFormatter.format(value);
}
...
function toggleTrend(trend) {
	return (host) => {
		host.trends = host.trends.includes(trend) ? host.trends.filter((t) => t !== trend) : [...host.trends, trend];
	};
}
var whotracksme_default = {
	...
	mergedStats: store_default(MergedStats, { id: ({ dateFrom, dateTo }) => ({
		dateFrom,
		dateTo
	}) }),
	resources: store_default(Resources),
	autoconsent: ({ resources, dateFrom, dateTo }) => {
		if (!store_default.ready(resources)) return 0;
	...
	trendsAggregate: 0,
	sankeyChartSlice: 20,
	render: ({ duration, dateFrom, dateTo, mergedStats, autoconsent, trends, trendsAggregate, sankeyChartSlice }) => html`
    <template layout>
      <main
      ...
      >
        <section layout="relative column gap:2 margin:2:0:0">
          <header layout="row items:center gap:1.5" translate="no">
            ...
            <div layout="column">
              <ui-text type="label-l" color="tertiary" uppercase> Ghostery </ui-text>
              <ui-text type="display-m">WhoTracks.Me</ui-text>
            </div>
          </header>
          <ui-text type="body-m" color="secondary" layout@768px:print="width:::75% margin:left:9">
            This privacy report was generated by tracking the trackers in a browser protected by
            Ghostery. The statistics here provide transparency about web tracking. To learn more
            about how Ghostery keeps its community safe, visit
            ${html`<a href="${WTM_PAGE_URL}" translate="no"> ghostery.com/whotracksme</a>`}.
          </ui-text>
          <div layout="column items:end gap" layout@768px:print="absolute top right">
            <div layout="row content:end gap">
              <ui-button onclick="${window.print}" layout="hidden" layout@1120px="block">
                <button>Print to PDF</button>
              </ui-button>
              <div layout="column" layout@print="hidden">
                ${duration !== "custom" && html`<ui-input layout="width::16">
        ...
        ${store_default.error(mergedStats)}
        ${store_default.ready(mergedStats) && html`
            ...
            <div layout="column gap:3" layout@1120px:print="padding">
              ...
              <div layout="column" layout@768px:print="row gap:2">
                ...
                <div layout="column grow">
                  ${mergedStats.groupedCategories.map(({ id, count }) => html`
              ...
              >
                ${mergedStats.groupedTrackers.map(({ id, name, category }, index) => html`
                    <ui-action>
          ...
          <section layout="column gap:3">
            <div layout="column gap:2 relative">
              ...
              ${mergedStats.groupedTrackers.length > 20 && html`
                <ui-input layout="absolute top:-1 right" layout@print="hidden">
                  <select value="${sankeyChartSlice}" onchange="${html.set("sankeyChartSlice")}">
                    <option value="20">Top 20</option>
                    ${mergedStats.groupedTrackers.length > 20 && html`<option value="50">Top 50</option>`}
                    ${mergedStats.groupedTrackers.length > 50 && html`<option value="100">Top 100</option>`}
                  </select>
                </ui-input>
              `}
            </div>
