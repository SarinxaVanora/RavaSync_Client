...
public sealed partial class CharaDataManager : DisposableMediatorSubscriberBase
{
    ...
    private readonly Dictionary<Guid, List<string>> _mcdfTempFiles = new();
    ...
    public async Task GetAllData(CancellationToken cancelToken)
    {
        ...
        UiBlockingComputation = GetAllDataTask = Task.Run(async () =>
        {
            ...
            if (_filesCdnUri != null && result.Any())
            {
                ...
                {
                    Logger.LogWarning(ex, "Error while prewarming CDN for own CharaData");
                }
            }
            ...
            return result
                ...
                .Select(k => new CharaDataFullExtendedDto(k))
                .ToList();
        });
    ...
    public void McdfApplyToTarget(string charaName)
    {
        ...
        UiBlockingComputation = McdfApplicationTask = Task.Run(async () =>
        {
            ...
            try
            {
                //resolve the actual in-game character, just like DownloadAndAplyDataAsync
                ...
                //create the handler by OBJECT INDEX, same as your normal CharaData path
                using GameObjectHandler? tempHandler = await _characterHandler
                    .TryCreateGameObjectHandler(chara.ObjectIndex)
                    .ConfigureAwait(false);
                ...
                lock (_mcdfTempFiles)
                {
                    _mcdfTempFiles[applicationId] = new List<string>(actuallyExtractedFiles);
                }

                // Merge FileSwaps into modPaths (gamePath -> filePath)
                foreach (var entry in charaFile.CharaFileData.FileSwaps
                             .SelectMany(k => k.GamePaths, (k, p) => new KeyValuePair<string, string>(p, k.FileSwapPath)))
                {
                ...
                await ApplyDataAsync(
                    ...
                    charaFile.CharaFileData.GlamourerData,
                    charaFile.CharaFileData.CustomizePlusData,
    ...
    CancellationToken token)
    {
        ...
        try
        {
            ...
            if (autoRevert)
            {
                ...
                await _ipcManager.Penumbra
                    .RemoveTemporaryCollectionAsync(Logger, applicationId, penumbraCollection)
                    .ConfigureAwait(false);
            }
            ...
            if (!string.IsNullOrEmpty(customizeData))
            {
                cPlusId = await _ipcManager.CustomizePlus
                    .SetBodyScaleAsync(tempHandler.Address, customizeData)
                    .ConfigureAwait(false);
            }
            else
            {
                // empty profile to clear C+
                cPlusId = await _ipcManager.CustomizePlus
        ...
        finally
        {
            ...
            if (autoRevert)
            {
                ...
                lock (_mcdfTempFiles)
                {
                    if (_mcdfTempFiles.TryGetValue(applicationId, out var list))
                    {
                        toDelete = list;
                        _mcdfTempFiles.Remove(applicationId);
                    }
                }
                ...
                if (toDelete != null)
                {
                    foreach (var file in toDelete)
                    {
                        ...
                        {
                            Logger.LogWarning(ex, "Failed to delete MCDF temp file {file}", file);
                        }
                    }
                }
            }
            ...
            if (metaInfo != null &&
                _configService.Current.FavoriteCodes.TryGetValue(metaInfo.Uploader.UID + ":" + metaInfo.Id, out var favorite) &&
                favorite != null)
            {
