...
public sealed class DiscoveryIntroUi : WindowMediatorSubscriberBase
{
    ...
    protected override void DrawInternal()
    {
        ...
        UiSharedService.ColorTextWrapped(
            "Discovery is an entirely optional extra on top of normal pairing.",
            ImGuiColors.DalamudViolet);

        UiSharedService.TextWrapped(
            "When you opt in, other RavaSync users who have also opted in can see that you're on RavaSync. " +
            ...
            "If you keep this off, you stay invisible to discovery. No hearts, no extra right-click option, " +
            "and people will need your UID to pair."
        );

        ImGuiHelpers.ScaledDummy(10);

        bool allowDiscovery = _tempDiscoveryOptIn;
        if (ImGui.Checkbox("Show my RavaSync presence to other RavaSync users", ref allowDiscovery))
        {
        ...
        ImGui.TextWrapped("You can change this again at any time in Settings - Discovery Settings.");
        ...
        {
            if (ImGui.Button("Done", new(120 * ImGuiHelpers.GlobalScale, 0)))
            {
