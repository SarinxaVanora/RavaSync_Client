...
{
    public const string MainServer = "Ravalyn's Domain";
    public const string MainServiceUri = "https://RavaSync.ravalyn.uk";
    ...
    private readonly SemaphoreSlim _tokenReconnectLock = new SemaphoreSlim(1, 1);
    private DateTime _lastTokenReconnectUtc = DateTime.MinValue;
    private volatile bool _tokenReconnectQueued = false;
    private long _hubEpoch = 0;
    public long HubEpoch => System.Threading.Interlocked.Read(ref _hubEpoch);
        ...
        TokenProvider tokenProvider, MareConfigService mareConfigService) : base(logger, mediator)
    {
        ...
        Mediator.Subscribe<ResumeMessage>(this, (msg) => _ = ResumeAsync(msg.UserData));
    ...
    public async Task<bool> CheckClientHealth()
    {
        ...
        {
            Logger.LogDebug(ex, "Health check skipped: hub not active");
            return false;
        }
        catch (Exception ex)
        {
            Logger.LogDebug(ex, "Health check failed");
            return false;
        }
    }
    ...
    public async Task CreateConnectionsAsync()
    {
        ...
        if (!_serverManager.CurrentServer.UseOAuth2)
        {
            ...
            if (multi)
            {
                ...
                _connectionDto = null;
                Mediator.Publish(new NotificationMessage("Multiple Identical Characters detected", "Your Service configuration has multiple characters with the same name and world set up. Delete the duplicates in the character management to be able to connect to RavaSync.",
        ...
        while (ServerState is not ServerState.Connected && !token.IsCancellationRequested)
        {
            ...
            try
            {
                ...
                Interlocked.Increment(ref _hubEpoch);
                InitializeApiHooks();
                ...
                if (_dalamudUtil.HasModifiedGameFiles)
                {
                    ...
                    if (!_mareConfigService.Current.DebugStopWhining || !RuntimeInformation.IsOSPlatform(OSPlatform.Linux))
                        Mediator.Publish(new NotificationMessage("Modified Game Files detected",
                            ...
                            "RavaSync, Penumbra, and some other plugins assume your FFXIV installation is unmodified in order to work. " +
    ...
    {
        // Use UID equality, and be null-safe
        var pair = _pairManager.PairsWithGroups.Keys
                .FirstOrDefault(p => string.Equals(p.UserData.UID, userData.UID, StringComparison.Ordinal));

        if (pair == null) 
        {
            Logger.LogInformation("{AliasorUID} — Not Found or offline, unpause failed",userData.AliasOrUID);
            return; // offline or not found — nothing to do
        }
    ...
    private async Task ClientHealthCheckAsync(CancellationToken ct)
    {
        try
        {
            while (!ct.IsCancellationRequested)
            {
                ...
                // When reconnecting/disconnected, InvokeAsync will throw. Just wait until we're connected again.
        ...
        {
            // normal on disconnect/reconnect/dispose
        }
        catch (Exception ex)
        {
            Logger.LogWarning(ex, "ClientHealthCheckAsync crashed");
        }
    }
    ...
    private void InitializeApiHooks()
    {
        ...
        OnUserPairRequestResult(dto => _ = Client_UserPairRequestResult(dto));

        _healthCheckTokenSource?.Cancel();
        _healthCheckTokenSource?.Dispose();
        _healthCheckTokenSource = new CancellationTokenSource();

        _healthCheckTask = Task.Run(() => ClientHealthCheckAsync(_healthCheckTokenSource.Token));
        _ = _healthCheckTask.ContinueWith(t =>
        {
            try
            {
                Logger.LogWarning(t.Exception, "Health check task faulted");
            }
            catch
            {
                // ignore
    ...
    public async Task LoadIninitialPairsAsync()
    {
    ...
    public async Task LoadOnlinePairsAsync()
    {
    ...
    private void MareHubOnReconnecting(Exception? arg)
    {
        ...
        if (arg != null)
            Logger.LogWarning(arg, "Connection interrupted... reconnecting");
        else
            Logger.LogWarning("Connection interrupted... reconnecting (no exception provided)");

        Mediator.Publish(new EventMessage(new Services.Events.Event(
    ...
    private async Task<bool> RefreshTokenAsync(CancellationToken ct)
    {
        try
        {
            ...
            if (!string.IsNullOrEmpty(token) && !string.Equals(token, _lastUsedToken, StringComparison.Ordinal))
            {
                Logger.LogDebug("Token rotated; scheduling reconnect to resume service.");
                _lastUsedToken = token;

                QueueTokenRotationReconnect();
        ...
        {
            return true; // normal during disconnect/dispose
        }
        catch (Exception ex)
        {
            Logger.LogWarning(ex, "Could not refresh token (keeping current connection)");
        }

        return false;
    }

    private void QueueTokenRotationReconnect()
    {
        if (_tokenReconnectQueued) return;
        _tokenReconnectQueued = true;

        _ = Task.Run(async () =>
        {
            try
            {
                if (ServerState is ServerState.Connecting or ServerState.Reconnecting or ServerState.Disconnecting)
                    return;
                await Task.Delay(TimeSpan.FromSeconds(2)).ConfigureAwait(false);

                await _tokenReconnectLock.WaitAsync().ConfigureAwait(false);
                try
                {
                    // Hard debounce: never more than once every 2 minutes
                    if (DateTime.UtcNow - _lastTokenReconnectUtc < TimeSpan.FromMinutes(2))
                        return;

                    _lastTokenReconnectUtc = DateTime.UtcNow;

                    // Maintenance reconnect, keep UI quiet
                    _doNotNotifyOnNextInfo = true;

                    Logger.LogInformation("Access token rotated; reconnecting to server");
                    await CreateConnectionsAsync().ConfigureAwait(false);
                }
                finally
                {
                    _tokenReconnectLock.Release();
                }
            }
            catch (Exception ex)
            {
                Logger.LogWarning(ex, "Token rotation reconnect failed");
            }
            finally
            {
                _tokenReconnectQueued = false;
            }
        });
    }
